...
(function($) {
  ...
  var orderRegExp = /-ORDER$/;
  var deleteRegExp = /-DELETE$/;
  
  var shouldRemoveForm = function(form){
    ...
    for(var i = 0; i < els.length; i++){
      ...
      if(!(orderRegExp.test(name) || deleteRegExp.test(name))){
        if(current.val() !== ""){
          isEmpty = false;
        }
      }
      if(orderRegExp.test(name) && current.is(':checked')){
        isEmpty = false;
      }
    }
  ...
  Formset.prototype.cleanEmptyForms = function() {
    this.$forms().each(function(){
      var $form = $(this);
      if(shouldRemoveForm($form)){
        $form.remove();
      }
    });

    this.$managementForm('TOTAL_FORMS').val(this.$forms().length)
    this.prefillOrder();
  }
